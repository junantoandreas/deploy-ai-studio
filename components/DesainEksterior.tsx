...
import { Upload, X, Eye, Download, Loader2, TreePine, Sparkles, PaintBucket, Fence } from 'lucide-react';
import { ImageFile, LoadingState } from '../types';
import { 
  generateExteriorStyleSuggestion, 
  generateExteriorInstruction, 
  generateExteriorDesign 
} from '../services/geminiService';

const DesainEksterior: React.FC = () => {
  // --- STATE ---
  ...
  // Inputs
  const [renovationType, setRenovationType] = useState('Mengubah Cat');
  ...
  // Constants
  const RENOVATION_OPTIONS = ['Mengubah Cat', 'Taman', 'Pagar', 'Kustom'];
  ...
  const handleSuggestStyle = async () => {
    if (!image) return;
    setLoadingText('Menganalisis bangunan...');
    setLoadingState(LoadingState.GENERATING);
    try {
      const suggestion = await generateExteriorStyleSuggestion(image);
  ...
  const handleGenerateInstruction = async () => {
    ...
    setLoadingState(LoadingState.GENERATING);
    try {
      const prompt = await generateExteriorInstruction(
        image, 
        renovationType, 
        customStyle
      );
  ...
  const handleGenerateDesign = async () => {
    ...
    setLoadingState(LoadingState.GENERATING);
    setLoadingText('Sedang merenovasi eksterior... (4 Variasi)');
    setResults([]);
    ...
    try {
      ...
      console.error(error);
      alert("Gagal merenovasi eksterior. Silakan coba lagi.");
      setLoadingState(LoadingState.ERROR);
    }
  };
  ...
  const downloadImage = (url: string, index: number) => {
    ...
    link.href = url;
    link.download = `desain-eksterior-${index + 1}-${Date.now()}.png`;
  ...
  const getRenovationIcon = (type: string) => {
    switch (type) {
        case 'Mengubah Cat': return <PaintBucket size={16} />;
        case 'Taman': return <TreePine size={16} />;
        case 'Pagar': return <Fence size={16} />;
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2 pb-20">
        ...
        <section>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Upload Eksterior</h3>
          </div>
          ...
          {!image ? (
            ...
            >
              ...
              <div className="text-center">
                <span className="text-sm font-medium block">Klik untuk Upload Foto Eksterior</span>
                <span className="text-xs text-gray-400 dark:text-gray-500">Max 1 Gambar (Tampak Depan/Belakang)</span>
              </div>
            </button>
          ) : (
            <div className="relative aspect-video w-full rounded-xl border border-gray-200 dark:border-dark-border overflow-hidden group bg-gray-50 dark:bg-dark-card flex justify-center">
              <img src={image.url} alt="Exterior" className="h-full object-contain" />
              <button 
        ...
        <section className={!image ? 'opacity-50 pointer-events-none' : ''}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Gaya Renovasi</h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            {RENOVATION_OPTIONS.map(opt => (
              ...
              >
                {getRenovationIcon(opt)}
                {opt}
              </button>
            ))}
          </div>

          {renovationType === 'Kustom' && (
            <div className="relative animate-in fade-in slide-in-from-top-2">
              <input
                type="text"
                value={customStyle}
                onChange={(e) => setCustomStyle(e.target.value)}
                placeholder="Gaya kustom (misal: Scandinavian, Industrial)..."
        ...
        <section className={!image ? 'opacity-50 pointer-events-none' : ''}>
           <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Instruksi Lanjutan</h3>
          </div>
          ...
          <div className="relative">
              <textarea
                ...
                placeholder="Instruksi tambahan (misal: Cat warna krem, taman dengan bunga mawar)..."
        ...
        >
          {loadingState === LoadingState.GENERATING ? (
            ...
            <>
              <TreePine size={20} />
              <span>Generate Desain Eksterior</span>
            </>
          )}
        </button>
      ...
      <div className="lg:w-[500px] xl:w-[600px] shrink-0 bg-gray-50 dark:bg-dark-sidebar rounded-3xl border border-gray-200 dark:border-dark-border p-6 flex flex-col transition-colors">
        <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-6">Hasil Renovasi Eksterior</h3>
          ...
          {results.length > 0 ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 text-center p-8 border-2 border-dashed border-gray-200 dark:border-dark-border rounded-2xl">
              ...
              <p className="text-sm max-w-[250px]">
                Hasil renovasi eksterior yang realistis akan muncul di sini (4 variasi).
              </p>
            </div>
          )}
        </div>
...
export default DesainEksterior;
