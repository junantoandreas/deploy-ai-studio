...
import { Upload, X, Eye, Download, ImagePlus, Loader2, Image as ImageIcon, RotateCcw } from 'lucide-react';
import { ImageFile, LoadingState } from '../types';
import { editRegionWithGemini } from '../services/geminiService';

const EditFoto: React.FC = () => {
  // State
  ...
  const startDrawing = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (ctx) {
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.strokeStyle = 'rgba(0, 255, 0, 0.5)'; // Green transparent for UI
      ctx.lineWidth = brushSize * (canvasRef.current!.width / containerRef.current!.clientWidth); 
    }
  };
  ...
  const handleEdit = async () => {
    if (!image || !canvasRef.current || !prompt) return;
    
    setLoadingState(LoadingState.GENERATING);
    setResult(null);

    try {
      // 1. Create mask from canvas
      const canvas = canvasRef.current;
      ...
      // Post-process: Make non-transparent pixels RED
      ...
      if(!hasPixels) {
          alert("Silakan tandai area gambar dengan brush terlebih dahulu.");
      ...
      // 2. Send to API
      const generatedImage = await editRegionWithGemini(image, maskDataUrl, prompt);
      setResult(generatedImage);
      setLoadingState(LoadingState.SUCCESS);
    } catch (error) {
      console.error(error);
      alert("Gagal mengedit foto. Silakan coba lagi.");
  ...
  return (
    <div className="h-auto md:h-full flex flex-col lg:flex-row gap-8">
      ...
      <div className="w-full lg:flex-1 flex flex-col gap-8 md:overflow-y-auto md:pr-2 pb-4 md:pb-20">
        ...
        <section className={`${!image ? 'opacity-50 pointer-events-none' : ''}`}>
           <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Tandai & Instruksi</h3>
          </div>
          ...
          <div className="bg-gray-50 dark:bg-dark-card p-4 rounded-xl border border-gray-100 dark:border-dark-border mb-6">
            ...
            <p className="text-xs text-gray-400 mt-2 mb-4">
              Usapkan brush pada objek yang ingin diubah.
            </p>
            ...
            <input
                ...
                placeholder='Contoh: "Tambah kacamata", "Ubah warna baju jadi biru"'
                className="w-full p-3 rounded-lg border border-gray-300 dark:border-dark-border dark:bg-dark-sidebar dark:text-white focus:ring-1 focus:ring-brand-blue focus:border-brand-blue outline-none text-sm placeholder-gray-400 dark:placeholder-gray-600"
            />
          </div>

          <button
            onClick={handleEdit}
            disabled={!image || !prompt || loadingState === LoadingState.GENERATING}
            className={`
              w-full py-4 rounded-xl font-semibold text-white shadow-lg flex items-center justify-center gap-3 transition-all
              ${!image || !prompt || loadingState === LoadingState.GENERATING
      ...
      <div className="w-full lg:w-[500px] xl:w-[600px] shrink-0 bg-gray-50 dark:bg-dark-sidebar rounded-3xl border border-gray-200 dark:border-dark-border p-6 flex flex-col transition-colors min-h-[400px]">
        <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-6">Hasil Edit Foto</h3>
        
        <div className="flex-1 flex flex-col items-center justify-center min-h-0">
          {result ? (
            <div className="w-full h-full flex flex-col gap-4">
               <div className="relative w-full flex-1 bg-white dark:bg-dark-card rounded-xl shadow-sm border border-gray-100 dark:border-dark-border overflow-hidden flex items-center justify-center group min-h-0">
            ...
            <div className="text-center text-gray-400 dark:text-gray-500 p-8 border-2 border-dashed border-gray-200 dark:border-dark-border rounded-2xl w-full">
              ...
              <p className="text-sm">
                Hasil edit foto akan muncul di sini.
              </p>
            </div>
          )}
        </div>
