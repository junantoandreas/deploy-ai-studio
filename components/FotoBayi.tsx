...
import { Upload, X, Eye, Download, Loader2, Baby, Sparkles, User } from 'lucide-react';
import { ImageFile, LoadingState } from '../types';
import { generateBabyPhoto, generateBabyThemeSuggestion } from '../services/geminiService';

const FotoBayi: React.FC = () => {
  // State
  ...
  const THEME_OPTIONS = ['Pastel', 'Keranjang', 'Studio Putih', 'Kustom'];
  ...
  const handleAISuggest = async () => {
    ...
    try {
      const suggestion = await generateBabyThemeSuggestion(image);
  ...
  const handleGenerate = async () => {
    ...
    setLoadingState(LoadingState.GENERATING);
    setLoadingText('Sedang membuat foto bayi... (4 variasi)');
    setResults([]);

    try {
      const promises = [1, 2, 3, 4].map(() => 
        generateBabyPhoto(image, gender, theme, customTheme, aspectRatio)
      );
  ...
  const downloadImage = (url: string, index: number) => {
    ...
    link.href = url;
    link.download = `foto-bayi-${index + 1}-${Date.now()}.png`;
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2 pb-20">
        ...
        <section>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Upload Foto Bayi</h3>
          </div>
          ...
          {!image ? (
            ...
            >
              ...
              <div className="text-center">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500">Max 1 Foto Bayi</span>
              </div>
            </button>
          ) : (
            <div className="relative aspect-video w-full rounded-xl border border-gray-200 dark:border-dark-border overflow-hidden group bg-gray-50 dark:bg-dark-card flex justify-center">
              <img src={image.url} alt="Baby" className="h-full object-contain" />
              <button 
                onClick={removeImage}
        ...
        <section className={!image ? 'opacity-50 pointer-events-none' : ''}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Pilihan Tema</h3>
          </div>
          ...
          {theme === 'Kustom' && (
            <div className="relative animate-in fade-in slide-in-from-top-2">
              <input
                ...
                placeholder="Masukkan tema manual (misal: Tema luar angkasa)..."
      ...
      <div className="lg:w-[500px] xl:w-[600px] shrink-0 bg-gray-50 dark:bg-dark-sidebar rounded-3xl border border-gray-200 dark:border-dark-border p-6 flex flex-col transition-colors">
        <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-6">Hasil Foto Bayi</h3>
          ...
          {results.length > 0 ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 text-center p-8 border-2 border-dashed border-gray-200 dark:border-dark-border rounded-2xl">
              ...
              <p className="text-sm max-w-[250px]">
                Hasil foto bayi yang lucu dan estetik akan muncul di sini (4 variasi).
              </p>
            </div>
          )}
        </div>
