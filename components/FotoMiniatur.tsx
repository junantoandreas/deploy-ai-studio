...
import { Upload, X, Eye, Download, Loader2, Minimize2, Sparkles, Image as ImageIcon } from 'lucide-react';
import { ImageFile, LoadingState } from '../types';
import { 
  generateMiniatureInstructionSuggestion, 
  generateMiniaturePhoto 
} from '../services/geminiService';

const FotoMiniatur: React.FC = () => {
  // --- STATE ---
  ...
  const handleSuggestInstruction = async () => {
    ...
    try {
      const suggestion = await generateMiniatureInstructionSuggestion(image);
  ...
  const handleGenerate = async () => {
    ...
    setLoadingState(LoadingState.GENERATING);
    setLoadingText('Membuat foto miniatur... (4 Variasi)');
    setResults([]);
    ...
    try {
      ...
      console.error(error);
      alert("Gagal membuat foto miniatur. Silakan coba lagi.");
      setLoadingState(LoadingState.ERROR);
    }
  };
  ...
  const downloadImage = (url: string, index: number) => {
    ...
    link.href = url;
    link.download = `foto-miniatur-${index + 1}-${Date.now()}.png`;
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2 pb-20">
          ...
          {!image ? (
            ...
            >
              ...
              <div className="text-center">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500">Max 1 Foto (Pemandangan/Kota/Objek)</span>
              </div>
            </button>
          ) : (
            <div className="relative aspect-video w-full rounded-xl border border-gray-200 dark:border-dark-border overflow-hidden group bg-gray-50 dark:bg-dark-card flex justify-center">
              <img src={image.url} alt="Uploaded" className="h-full object-contain" />
              <button 
        ...
        <section className={!image ? 'opacity-50 pointer-events-none' : ''}>
           <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Perintah Miniatur</h3>
          </div>
          ...
          <div className="relative">
              <textarea
                ...
                placeholder="Deskripsikan efek miniatur (misal: Buat seperti kota mainan lego dengan efek tilt-shift)..."
                className="w-full p-4 rounded-xl bg-gray-50 dark:bg-dark-card border border-gray-200 dark:border-dark-border dark:text-white focus:border-brand-blue focus:ring-1 focus:ring-brand-blue outline-none text-sm min-h-[80px] resize-none pr-28 placeholder-gray-400 dark:placeholder-gray-600"
              />
              <button
      ...
      <div className="lg:w-[500px] xl:w-[600px] shrink-0 bg-gray-50 dark:bg-dark-sidebar rounded-3xl border border-gray-200 dark:border-dark-border p-6 flex flex-col transition-colors">
        <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-6">Hasil Foto Miniatur</h3>
        
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {results.length > 0 ? (
            <div className="grid grid-cols-2 gap-4">
              {results.map((res, idx) => (
                <div key={idx} className="group relative w-full bg-white dark:bg-dark-card rounded-xl shadow-sm border border-gray-100 dark:border-dark-border overflow-hidden flex items-center justify-center">
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 text-center p-8 border-2 border-dashed border-gray-200 dark:border-dark-border rounded-2xl">
              ...
              <p className="text-sm max-w-[250px]">
                Efek miniatur yang unik dan menarik akan muncul di sini (4 variasi).
              </p>
            </div>
          )}
        </div>
