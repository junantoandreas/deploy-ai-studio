...
const HapusLatar: React.FC = () => {
  ...
  const handleRemoveBackground = async () => {
    ...
    try {
      // Generate 4 variations as requested (sometimes models produce slightly different isolations)
      const promises = [1, 2, 3, 4].map(() => 
        removeBackgroundWithGemini(image)
      );
  ...
  const downloadImage = (url: string, index: number) => {
    ...
    link.href = url;
    link.download = `hapus-latar-${index + 1}-${Date.now()}.png`;
      ...
      <div className="flex-1 flex flex-col gap-8 overflow-y-auto pr-2 pb-20">
          ...
          <div className="w-full">
            {image ? (
              <div className="relative aspect-video w-full rounded-xl border border-gray-200 dark:border-dark-border overflow-hidden group bg-gray-50 dark:bg-dark-card flex justify-center">
                <img src={image.url} alt="Original" className="h-full object-contain" />
                <button 
                  onClick={removeImage}
                  className="absolute top-2 right-2 bg-white/90 p-2 rounded-full text-red-500 hover:bg-red-50 transition-colors shadow-sm"
                >
                  <X size={16} />
                </button>
              </div>
        ...
        <section>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Proses</h3>
          </div>
          ...
          >
            {loadingState === LoadingState.GENERATING ? (
              <>
                ...
                <span>Memproses Background...</span>
              </>
            ) : (
              <>
                <Scissors size={20} />
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {results.length > 0 ? (
            <div className="grid grid-cols-2 gap-4">
              {results.map((res, idx) => (
                <div key={idx} className="group relative aspect-square bg-[url('https://media.istockphoto.com/id/1145610818/vector/transparent-background-seamless-pattern.jpg?s=612x612&w=0&k=20&c=6_B8HqXoZ_bH7JqQ5B6d_k9Z5Q8z_a1_X9x_j7x_x8=')] bg-cover rounded-xl shadow-sm border border-gray-100 dark:border-dark-border overflow-hidden">
                  {/* Using a checkerboard pattern background for the container to visualize transparency if supported, though model output is usually white bg */}
                  <img src={res} alt={`Result ${idx}`} className="w-full h-full object-contain" />
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 text-center p-8 border-2 border-dashed border-gray-200 dark:border-dark-border rounded-2xl">
              ...
              <p className="text-sm max-w-[250px]">
                Upload gambar dan klik tombol Hapus Background untuk melihat hasilnya di sini.
              </p>
            </div>
          )}
        </div>
