...
import { Upload, X, Eye, Download, Loader2, Brush, Sparkles, Image as ImageIcon, PenTool } from 'lucide-react';
import { ImageFile, LoadingState } from '../types';
import { 
  generateArtStyleSuggestion, 
  generateArtInstruction, 
  generateArtisticImage 
} from '../services/geminiService';

const SeniLukis: React.FC = () => {
  // --- STATE ---
  ...
  // Inputs
  const [style, setStyle] = useState('Cat Air');
  ...
  // Constants
  const STYLE_OPTIONS = ['Cat Air', 'Digital Art', 'Lukisan Minyak', 'Sketsa Pensil', 'Kustom'];
  const RATIO_OPTIONS = ['1:1', '3:4', '9:16', '16:9'];
  ...
  const handleSuggestStyle = async () => {
    ...
    try {
      const suggestion = await generateArtStyleSuggestion(image);
  ...
  const handleGenerateInstruction = async () => {
    ...
    setLoadingState(LoadingState.GENERATING);
    try {
      const prompt = await generateArtInstruction(
        image, 
        style, 
        customStyle
      );
  ...
  const handleGenerateArt = async () => {
    ...
    setLoadingState(LoadingState.GENERATING);
    setLoadingText('Sedang melukis... (4 Variasi)');
    setResults([]);
    ...
    try {
      ...
      console.error(error);
      alert("Gagal membuat seni lukis. Silakan coba lagi.");
      setLoadingState(LoadingState.ERROR);
    }
  };
  ...
  const downloadImage = (url: string, index: number) => {
    ...
    link.href = url;
    link.download = `seni-lukis-${index + 1}-${Date.now()}.png`;
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1 flex flex-col gap-6 overflow-y-auto pr-2 pb-20">
        ...
        <section>
          ...
          {!image ? (
            ...
            <div className="relative aspect-video w-full rounded-xl border border-gray-200 dark:border-dark-border overflow-hidden group bg-gray-50 dark:bg-dark-card flex justify-center">
              <img src={image.url} alt="Input" className="h-full object-contain" />
              <button 
        ...
        <section className={!image ? 'opacity-50 pointer-events-none' : ''}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-gray-800 dark:text-gray-100">Pilih Gaya Seni</h3>
          </div>
          ...
          {style === 'Kustom' && (
            <div className="relative animate-in fade-in slide-in-from-top-2">
              <input
                ...
                placeholder="Gaya seni kustom (misal: Impressionism, Pop Art)..."
        ...
        <section className={!image ? 'opacity-50 pointer-events-none' : ''}>
          ...
          <div className="relative">
              <textarea
                ...
                placeholder="Detail tambahan (misal: Buat warna lebih cerah, tambahkan efek cahaya)..."
      ...
      <div className="lg:w-[500px] xl:w-[600px] shrink-0 bg-gray-50 dark:bg-dark-sidebar rounded-3xl border border-gray-200 dark:border-dark-border p-6 flex flex-col transition-colors">
        <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-6">Hasil Karya Seni</h3>
          ...
          {results.length > 0 ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 text-center p-8 border-2 border-dashed border-gray-200 dark:border-dark-border rounded-2xl">
              ...
              <p className="text-sm max-w-[250px]">
                Hasil transformasi seni lukis akan muncul di sini (4 variasi).
              </p>
            </div>
          )}
        </div>
